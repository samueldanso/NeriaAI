    ...
    <Link href={`/capsule/${capsule.id}`}>
      <div className="card-base hover:bg-surface-hover transition-colors cursor-pointer h-full flex flex-col group">
        <div className="flex items-start justify-between mb-3">
          <span className="text-xs px-2 py-1 rounded bg-primary/20 text-primary font-medium">{capsule.topic}</span>
        ...
        {capsule.description && <p className="text-sm text-muted mb-3 line-clamp-2">{capsule.description}</p>}
        ...
        <Button size="sm" className="w-full bg-primary text-background hover:bg-primary-dark">
          View Details
        </Button>
      </div>
    </Link>
