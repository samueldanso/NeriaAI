...
export function MessageBubble({ message, isUser, onExpandReasoning }: MessageBubbleProps) {
      ...
      <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
        <div
          className={`max-w-md p-4 rounded-lg ${
            isUser ? "bg-primary text-background" : "bg-surface border border-border text-foreground"
          }`}
        >
          ...
          {!isUser && (
            <div className="mt-3 flex gap-2 flex-wrap">
              ...
              {message.capsuleId && (
                <Button size="sm" className="text-xs h-7 bg-accent text-background hover:bg-accent/80">
