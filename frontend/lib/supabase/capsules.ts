...
): Promise<{ success: true; capsule: KnowledgeCapsule } | { success: false; error: string }> {
	try {
...
export async function getCapsules(
	limit: number = 10,
	offset: number = 0
): Promise<{ success: true; capsules: KnowledgeCapsule[] } | { success: false; error: string }> {
	try {
		const supabase = createClient()
...
): Promise<{ success: true; results: CapsuleSearchResult[] } | { success: false; error: string }> {
	try {
		const supabase = createClient()
