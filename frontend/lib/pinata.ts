...
export async function uploadCapsuleToIPFS(
  ...
  | { success: true; ipfsHash: string; gatewayUrl: string }
...
export async function getCapsuleFromIPFS(
  ...
  { success: true; data: CapsuleIPFSData } | { success: false; error: string }
> {
  try {
    const response = await fetch(
      `${env.NEXT_PUBLIC_GATEWAY_URL}/ipfs/${ipfsHash}`,
    );
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "Failed to retrieve from IPFS",
    };
  }
}
