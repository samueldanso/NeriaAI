...
import { getMintingConfig, type MintingMode, shouldAutoMint } from './minting-options'
import { mintKnowledgeCapsuleNFT } from './nft-minting'
import { type CreateCapsuleInput, createCapsule, updateCapsuleIPFS } from './supabase'
...
export async function processCapsuleStorage(
	...
	try {
		...
		const shouldAutoMintNFT = shouldAutoMint(capsuleData.confidence, mintingConfig)

		let nftResult: any = { success: false }

		if (shouldAutoMintNFT) {
			console.log('⛓️ Step 4: Auto-minting NFT on Base L2...')
			nftResult = await mintKnowledgeCapsuleNFT(
				ipfsResult.ipfsHash,
				capsuleData.walletProvider,
				capsuleData.creator_address
			)

			if (!nftResult.success) {
				console.warn('⚠️ Auto-minting failed, but storage completed:', nftResult.error)
			} else {
			...
			console.log('⛓️ Step 4: Manual minting required (confidence below threshold)')
		}
